...
uses
  System.SysUtils, Generics.Collections, Seven.SimpleParser.Lexer.Types, Seven.DelphiAST.Consts;
...
type
  ...
  IASTVisitor = interface; // Marcelo Melo - 01/05/2025
  ...
  public
    ...
    procedure Accept(const AVisitor: IASTVisitor); virtual; // Marcelo Melo - 01/05/2025

    function Clone: TSyntaxNode; virtual;
    procedure AssignPositionFrom(const Node: TSyntaxNode);

    function GetAttribute(const Key: TAttributeName): string; overload;
    function HasAttribute(const Key: TAttributeName): Boolean; overload;
    procedure SetAttribute(const Key: TAttributeName; const Value: string); overload;
  ...
  // Marcelo Melo - 01/05/2025
  IASTVisitor = interface
  ['{5B06123F-C1C0-4381-B0A6-A5B104082345}']
    procedure Visit(const ANode: TSyntaxNode);
  end;
...
// Marcelo Melo - 01/05/2025
procedure TSyntaxNode.Accept(const AVisitor: IASTVisitor);
begin
  AVisitor.Visit(Self);
end;
