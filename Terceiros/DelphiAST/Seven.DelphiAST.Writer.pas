...
uses
  ...
  {$ENDIF}
  System.Classes,
  Seven.DelphiAST.Classes, System.SysUtils;
...
uses
  ...
  {$IFNDEF FPC}
    Seven.DelphiAST.Serialize.Binary,
  {$ENDIF}
...
class procedure TSyntaxTreeWriter.NodeToXML(const Builder: TStringBuilder;
  ...
  begin
    ...
    Builder.Append(' node_class="' + Node.ClassName + '"');
    Builder.Append(' children="' + IntToStr(Length(Node.ChildNodes)) + '"');
    Builder.Append(' atributes="' + IntToStr(Length(Node.Attributes)) + '"');
